...
public class AddQuestionActivity extends AppCompatActivity {
    ...
    private void upload() {
        ...
        map.put("question", question.getText().toString().trim());
        map.put("setId", setId);
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    QuestionModel questionModel = new QuestionModel(id, map.get("question").toString()
                            ...
                            , map.get("optionD").toString(), map.get("correctANS").toString(), setId);
                    if(position != -1){
                    ...
                    Toast.makeText(AddQuestionActivity.this, "Something went wrong!" + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
    ...
    private void setData() {
        ...
        for (int i = 0; i < answers.getChildCount(); i++) {
            if (((EditText) answers.getChildAt(i)).getText().toString().equals(questionModel.getAns())) {
                ((RadioButton) options.getChildAt(i)).setChecked(true);
                break;
            }
        }
    }
